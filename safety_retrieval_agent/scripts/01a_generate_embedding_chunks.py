#!/usr/bin/env python
"""Generate embeddings in resumable chunks.

Run locally or as an Azure ML command job with no arguments:

    python scripts/01a_generate_embedding_chunks.py
...
from safety_retrieval_agent.index_pipeline import generate_embedding_chunks
...
def main() -> dict:
    ...
    diagnostics = configure_cpu_runtime(settings, include_faiss=False)
    summary = generate_embedding_chunks(settings)
